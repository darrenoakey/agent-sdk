# greenline — daz-agent-sdk

This repo uses **greenline**: a local, serialized, gated CI/CD tool. There is one
canonical checkout of `daz-agent-sdk`; it is always on `main`, always clean,
and always identical to what prod (`daz-agent-sdk`) runs. Every change reaches
...
```
greenline worktree feature-x        # new worktree at /Volumes/Gumby/worktrees/greenline/daz-agent-sdk/feature-x
cd /Volumes/Gumby/worktrees/greenline/daz-agent-sdk/feature-x         # branch gl/feature-x off last-green
# ...edit, test, commit...
greenline submit                     # gate: squash-merge -> check -> ff main -> deploy -> publish
greenline done                       # remove the worktree + branch once merged
```

### `greenline submit` must never be held inside an agent turn

A submit is minutes of `check` + a real `deploy`. An agent that runs `greenline
submit` in the foreground, or polls it in a busy-wait loop, burns its whole
...
- **`./run deploy`** — cwd = the canonical checkout. Rebuild/restart prod (e.g.
  `auto -q restart daz-agent-sdk`), **health-check**, exit nonzero on unhealthy, and be
